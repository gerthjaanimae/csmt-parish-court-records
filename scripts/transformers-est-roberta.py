...
for n in range(num_iterations):
    ...
    for location in data:
        ...
        model_args.output_dir=os.path.join("est-roberta-outputs", location)
        ...
        model = NERModel(
            ...
            "roberta", "EMBEDDIA/est-roberta", args=model_args, use_cuda=False
        )
        ...
        diag_lines=[]
        if not os.path.exists(os.path.join("..", "diagnostics-est-roberta", str(n+1))):
            os.makedirs(os.path.join("..", "diagnostics-est-roberta", str(n+1)))
        diag_filepath=os.path.join("..", "diagnostics-est-roberta", str(n+1), location+".txt")
        vabamorf_correct=0
        estroberta_correct=0
        sentences=[]
        sentence=[]
        ...
        result['estroberta']= estroberta_correct
        result['estroberta_percentage']=round((estroberta_correct/result['total']), 2)
        print (result)
        #print(len_preds, len(eval))
        with open(diag_filepath, "w") as fout:
            fout.write("word\tcorrect label\tvabamorf\test-roberta\n")
